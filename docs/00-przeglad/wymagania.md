...
| FR-07.08 | Profil i preferencje: waluta bazowa (domyślnie PLN), metoda cost basis, ustawienia widoku podatkowego (dzień przychodu, koszt przewalutowania), strefa czasowa, język, motyw (ciemny domyślnie, jasny, systemowy), paleta zysk/strata przyjazna daltonistom. | P1 | Zmiana palety działa globalnie bez przeładowania; preferencje synchronizowane między urządzeniami. | 3.2 |
...
| Z-15 | §3.1.2 portfel z XTB | Obok akcji i ETF XTB oferuje CFD (dźwignia, punkty swapowe, korekty dywidendowe) — wymagają osobnego modelu. Koszt przewalutowania w XTB (0,5 % kursu — potwierdzone w Kroku 4, [`../03-dane/obliczenia-finansowe.md`](../03-dane/obliczenia-finansowe.md) § 12.7) wpływa na koszt nabycia. | MVP: akcje i ETF (także na IKE/IKZE); wiersze CFD rozpoznawane i oznaczane jako nieobsługiwane (nie gubione); koszt przewalutowania wliczany do kosztu nabycia. | FR-03.01. |
