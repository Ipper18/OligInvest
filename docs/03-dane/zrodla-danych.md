...
1. **GPW EOD:** oficjalne archiwum GPW zwraca jednym żądaniem XLS z całym rynkiem za dany dzień — to primary (klient identyfikuje się uczciwie, 1 żądanie/dzień, bez obchodzenia jakichkolwiek blokad). Stooq zabrania automatów (`robots.txt` + challenge JS) — tylko ręczny import.
...
|---|---|---|---|---|---|---|---|---|
| **GPW — Archiwum notowań** ✅ | Akcje GPW (cały rynek w jednym pliku/dzień; parametr `type=10` = akcje; kody dla ETF/indeksów/obligacji ❓ do zmapowania). Kolumny (zweryfikowane na pliku z 2026-09-16, 402 instrumenty): Data, Nazwa (skrócona, nie ticker), ISIN, Waluta, Kurs otwarcia/max/min/zamknięcia, Zmiana, Wolumen, Liczba Transakcji, Obrót (**tys. zł**), Liczba i Wartość otwartych pozycji, Cena nominalna — szczegóły w `formaty-importu.md` § 4. Pliki dzienne zawierają też spółki później wycofane → uniwersum point-in-time dla backtestów. | Brak opublikowanego limitu; 1 żądanie = ~125 KB (XLS BIFF). `robots.txt` nie zabrania ścieżki archiwum; test 2026-09-18: klient z **identyfikującym** `User-Agent` (`OligInvest/…`) → HTTP 200, `application/vnd.ms-excel`. Zasady: 1 żądanie na dzień sesyjny, uczciwa identyfikacja klienta, **żadnego obchodzenia blokad** — jeśli GPW zacznie odrzucać automat, przechodzimy na import ręczny (FR-08.09) | EOD (po sesji) | Nie | ❓ Regulamin serwisu dla archiwum nieodnaleziony (płatne produkty danych GPW mają osobny regulamin: tylko na własne potrzeby, bez dalszej dystrybucji); przyjmujemy: użytek prywatny, **bez redystrybucji** | Wysoka (archiwum od lat), format XLS może się zmienić → parser z testem kontraktowym | **PRIMARY EOD GPW** (batch nocny, backfill historii dzień po dniu) | https://www.gpw.pl/archiwum-notowan (`?fetch=1&type=10&instrument=&date=DD-MM-YYYY`) |
...
| **Stooq** ✅ | Akcje/indeksy GPW i świat, CSV historyczne | Endpoint CSV zwraca **challenge JS (proof-of-work)**, a `robots.txt` zabrania dostępu wszystkim botom poza Googlebot/Bingbot (`User-agent: * Disallow: /`) — **automatyczne pobieranie niedozwolone** (test 2026-09-18) | EOD | Nie | ❓ Regulamin Stooq; użytek osobisty | Nie dotyczy (tylko ręcznie) | **Ręczny import CSV** przez użytkownika (fallback/backfill historii sprzed 2000 r.) | https://stooq.pl/ |
...
| **EODHD** ✅ | Free: **20 zapytań/dobę**, ❓ tylko wybrane tickery (AAPL, TSLA, …) i 1 rok; plan „All World” (GPW) **19,99 USD/mies.** | 20/dobę, 1 000/min | EOD | Nie (free trial bez karty) | Free do testów; komercyjne plany płatne | Wysoka | ⛔ free bezużyteczny; **jedyna sensowna płatna opcja dla GPW EOD** → [`../10-ograniczenia.md`](../10-ograniczenia.md) L-12 | https://eodhd.com/pricing |
...
| EODHD free | 20 zapytań/dobę i ograniczone tickery; opcja płatna opisana w [`../10-ograniczenia.md`](../10-ograniczenia.md) (L-12) |
