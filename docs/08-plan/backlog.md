...
- **2026-09-22, BL-003:** wykonane lokalnie: graf manifestów, cykle, publiczne eksporty i importy produkcyjne (parser istniejącego TypeScript); 28 testów PASS, w tym CLI odrzucające niedozwolony import. `pnpm check:deps`, lint skryptów, testy skryptów i kontrola dokumentacji PASS. Dodany workflow `modules`; status `w toku` do wspólnego DoD. Estymacja 1 d bez zmian; odchylenie niezmierzone, bez nowego ADR/ryzyka.
